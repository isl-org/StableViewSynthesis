...
def get_db_logger(db_path):
    db_logger = sqlite.Logger(db_path)
    db_logger.add_table(
        sqlite.Table(
            "metrics",
            fields=[
...
class TrainDataset(torch.utils.data.Dataset):
    def __init__(self, dset, n_train_iters, batch_size, train_iter=0):
        self.dset = dset
    ...
    def __len__(self):
        return (self.n_train_iters * self.batch_size) - self.train_iter

    def __getitem__(self, idx):
        rng = dataset_rng(self.train_iter + idx)
        idx = int(rng.randint(0, self.n_train_iters * self.batch_size))
...
class Worker(object):
    ...
    def setup_experiment(self):
        ...
        self.db_logger = get_db_logger(self.db_path)
    ...
    def metric_add_train(self, iter, metric, value, method_suffix=None):
        if method_suffix:
            method = f"{self.experiment_name}_{method_suffix}"
        else:
    ...
    def metric_add_eval(self, iter, dataset, metric, value, method_suffix=None):
        if method_suffix:
            method = f"{self.experiment_name}_{method_suffix}"
        else:
            method = self.experiment_name
    ...
    def slurm(self, args):
        ...
        with open(slurm_sh_path, "w") as fp:
            ...
            _write(fp, f"#SBATCH --qos {args.slurm_qos}")
    ...
    def get_train_data_loader(self, dset, iter):
        return torch.utils.data.DataLoader(
            TrainDataset(dset, self.n_train_iters, self.train_batch_size, iter),
    ...
    def get_net_paths(self, net_root=None):
        ...
        for net_path in sorted(net_root.glob("net_*.params")):
            ...
            iters.append(str(iter))
        if len(iters) > 0:
            net_paths["first"] = net_paths[iters[0]]
            net_paths["last"] = net_paths[iters[-1]]
        return net_paths
    ...
    def eval_iters(self, worker_objects, iters=None, net_root=None):
        ...
        if iters is None or len(iters) == 0:
            iters = [iter for iter in net_paths.keys() if iter not in ['first', "last"]]
    ...
    def train(self, worker_objects, resume=False):
        logging.info(f"Get data sets")
        train_set = self.get_train_set()
        eval_sets = self.get_eval_sets()

        # get worker objects
        logging.info(f"Setup net and optimizer")
        net = worker_objects.get_net()
        ...
        logging.info(f"load state if existent")
        iter = 0
        ...
        logging.info(f"Setup training data loader and other stuff")
        train_loader = self.get_train_data_loader(train_set, iter)
        iter_range = range(iter, self.n_train_iters)
        n_iter_range = self.n_train_iters - iter
        eta_total = utils.ETA(length=n_iter_range)
        mean_loss = utils.CumulativeMovingAverage()
        net.train()
        optimizer.zero_grad()
        ...
        for iter, data in zip(iter_range, train_loader):
            ...
            if do_save or iter == self.n_train_iters - 1:
                # store network
            ...
            # eval network
            if do_eval or iter == self.n_train_iters -1:
                self.eval(
